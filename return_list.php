    ...
    <form action="#" method="post" enctype="multipart/form-data">
      <table class="my-table" id="my-table" border="0">
        ...
        <tbody>
          <?php
            $sno=0;
		    $sql = "SELECT student.studentname,book.bookname,return.admission_no,return.bookcode,return.issue_date,return.fine,return.doc FROM `student`,`book`,`return` WHERE return.admission_no=student.admission_no AND return.bookcode=book.bookcode";
...
function searching() {
  ...
  for (i=0; i<tr.length; i++) {
    ...
    if (td) {
      ...
      if ((txtValue.toUpperCase().indexOf(filter) > -1) || (td1.innerHTML.toUpperCase().indexOf(filter) > -1) || (td2.innerHTML.toUpperCase().indexOf(filter) > -1) || (td3.innerHTML.toUpperCase().indexOf(filter) > -1) || (td4.innerHTML.toUpperCase().indexOf(filter) > -1) || (td5.innerHTML.toUpperCase().indexOf(filter) > -1)) {
        tr[i].style.display = "";
      } 
	  else {
